  ...
  if ($this_is_home_page == 'true' and CAROUSEL_FEATURED_PRODUCTS == 'true') {
    ...
    if (CAROUSEL_FEATURED_LINK == 'true') {	
      if (zen_has_product_attributes($featured_products->fields['products_id'])) {
        $buy_now_link = '<div class="more-info"><a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']) . '&products_id=' . $featured_products->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a></div>';
      } else {
